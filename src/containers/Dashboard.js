...
// bug dashboard
        bills.forEach((bill) => {
            let billCard = document.querySelector(`#open-bill${bill.id}`);

            if (!this.eventAllreadyAdded.includes(billCard)) {
                $(`#open-bill${bill.id}`).click((e) =>
                    this.handleEditTicket(e, bill, bills)
                );

                this.eventAllreadyAdded.push(billCard);
            }
        });
